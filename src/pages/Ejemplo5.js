...
const DatosApi = (props) => {
    ...
    useEffect(() => {
        const cargarDatos = async () => {
            setCargado(true);
            // res es la variable que creo para cargar los personajes
            const res = await (await fetch('https://rickandmortyapi.com/api/character')).json();
            setPersonajes(res.results);
            setCargado(false);
        }
        // fetch:para poder traer datos nativos de JS y otros ejemplos pueden ver que tambien se utiliza axios(peticiones)
        cargarDatos();
    }, []);

    return (
        <>
            <h1>carga de datos atravez de una api</h1>
